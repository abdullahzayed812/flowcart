...
export class MySQLQueryExecutor {
  public async query<T extends QueryResult = RowDataPacket[]>(sql: string, params?: any[]): Promise<T> {
    const conn = MySQLConnection.getInstance();
    const [result] = await conn.query<T>(sql, params);
    return result;
  }

  public async execute<T extends QueryResult = ResultSetHeader>(sql: string, params?: any[]): Promise<T> {
    const conn = MySQLConnection.getInstance();
